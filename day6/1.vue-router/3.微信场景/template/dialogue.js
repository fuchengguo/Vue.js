var dialogue = {
	template:`
		<div id="app" _v-c8294afa="">
			<header class="app-header" _v-c8294afa="">
				<div class="_effect _effect--50" _v-c8294afa="">
					<div class="_cover-top" _v-60c89da3="" _v-c8294afa="" style="overflow:visible">
						<div class="other" _v-60c89da3=""> <span class="iconfont icon-tips-jia" _v-60c89da3="" style="touch-action: manipulation; user-select: none; -webkit-user-drag: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0); display: none;"></span> <span class="iconfont icon-tips-add-friend" _v-60c89da3="" style="display: none;"></span>
							<ul class="tips-menu tips-close" _v-60c89da3="">
								<li _v-60c89da3=""> <span class="iconfont icon-tips-xiaoxi" _v-60c89da3=""></span>
									<div _v-60c89da3="">发起群聊</div>
								</li>
								<li _v-60c89da3=""> <span class="iconfont icon-tips-add-friend" _v-60c89da3=""></span>
									<div _v-60c89da3="">添加朋友</div>
								</li>
								<li _v-60c89da3=""> <span class="iconfont icon-tips-saoyisao" _v-60c89da3=""></span>
									<div _v-60c89da3="">扫一扫</div>
								</li>
								<li _v-60c89da3=""> <span class="iconfont icon-tips-fukuan" _v-60c89da3=""></span>
									<div _v-60c89da3="">收付款</div>
								</li>
							</ul>
							<div class="tips-masker" _v-60c89da3="" style="display: none;"></div>
						</div>
						<div class="center" _v-60c89da3=""> 微信 <span class="parentheses" _v-60c89da3="">12</span> </div>
					</div>
				</div>
			</header>
			<section class="app-content" _v-c8294afa="">
				<div class="_full_inner _scroll _effect component-chat _effect--30" _v-c7cde232="">
					<div class="weui_search_bar" _v-c7cde232="">
						<form class="weui_search_outer">
							<div class="weui_search_inner"> <i class="weui_icon_search"></i> <input type="search" class="weui_search_input" placeholder="搜索">
								<a href="javascript:" class="weui_icon_clear" style="display: none;"></a>
							</div> <label class="weui_search_text"> <i class="weui_icon_search"></i> <span>搜索</span> </label> </form>
						<a href="javascript:" class="weui_search_cancel">取消</a>
					</div>
					<ul class="wechat-list" _v-c7cde232="">
						<li class="item _line-fine chat-item-transition" _v-c7cde232="">
							<div class="info" _v-c7cde232="" style="touch-action: pan-y; user-select: none; -webkit-user-drag: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);">
								<div class="ico-box" _v-c7cde232=""> <i _v-c7cde232="" class="_news-count">1</i>
									<div class="ico" _v-c7cde232=""> <img alt="pic" _v-c7cde232="" src="//ad-gold-cdn.xitu.io/14999138688354f1720f589d2d33db77f026bb07c8f67.jpg"> </div>
								</div>
								<div class="desc" _v-c7cde232="">
									<div class="desc-time" _v-c7cde232="">16:26</div>
									<div class="desc-title" _v-c7cde232="">杨涛</div>
									<div class="desc-message" _v-c7cde232="">
										<div class="desc-mute iconfont icon-mute" _v-c7cde232="" title="false" style="display: none;"></div> <span _v-c7cde232="" title="friends">杨涛:</span> <span _v-c7cde232="">晚上打球?</span> </div>
								</div>
							</div>
							<div class="handle" _v-c7cde232="">
								<div class="handle-unread" _v-c7cde232="" style="display: none; touch-action: manipulation; user-select: none; -webkit-user-drag: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);">标为未读</div>
								<div class="handle-unread" _v-c7cde232="" style="touch-action: manipulation; user-select: none; -webkit-user-drag: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);">标为已读</div>
								<div class="handle-del" _v-c7cde232="" style="touch-action: manipulation; user-select: none; -webkit-user-drag: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);">删除</div>
							</div>
						</li>
						<li class="item _line-fine chat-item-transition" _v-c7cde232="">
							<div class="info" _v-c7cde232="" style="touch-action: pan-y; user-select: none; -webkit-user-drag: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);">
								<div class="ico-box" _v-c7cde232=""> <i _v-c7cde232="" class="_news-dot">1</i>
									<div class="ico" _v-c7cde232=""> <img alt="pic" _v-c7cde232="" src="//ad-gold-cdn.xitu.io/1499913766441c45960d8bd44ed5ffcf0d4a67d0aa317.jpg"> </div>
								</div>
								<div class="desc" _v-c7cde232="">
									<div class="desc-time" _v-c7cde232="">16:26</div>
									<div class="desc-title" _v-c7cde232="">微信群01</div>
									<div class="desc-message" _v-c7cde232="">
										<div class="desc-mute iconfont icon-mute" _v-c7cde232="" title="true"></div> <span _v-c7cde232="" title="group" style="display: none;">领导:</span> <span _v-c7cde232="">大家下班的时候记得锁好门</span> </div>
								</div>
							</div>
							<div class="handle" _v-c7cde232="">
								<div class="handle-unread" _v-c7cde232="" style="display: none; touch-action: manipulation; user-select: none; -webkit-user-drag: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);">标为未读</div>
								<div class="handle-unread" _v-c7cde232="" style="touch-action: manipulation; user-select: none; -webkit-user-drag: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);">标为已读</div>
								<div class="handle-del" _v-c7cde232="" style="touch-action: manipulation; user-select: none; -webkit-user-drag: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);">删除</div>
							</div>
						</li>
						<li class="item _line-fine chat-item-transition" _v-c7cde232="">
							<div class="info" _v-c7cde232="" style="touch-action: pan-y; user-select: none; -webkit-user-drag: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);">
								<div class="ico-box" _v-c7cde232=""> <i _v-c7cde232="" class="_news-count">11</i>
									<div class="ico" _v-c7cde232=""> <img alt="pic" _v-c7cde232="" src="//ad-gold-cdn.xitu.io/14999139497307c668d55b4dd5984b6accc6bf41206a8.jpg"> </div>
								</div>
								<div class="desc" _v-c7cde232="">
									<div class="desc-time" _v-c7cde232="">16:26</div>
									<div class="desc-title" _v-c7cde232="">稀土圈</div>
									<div class="desc-message" _v-c7cde232="">
										<div class="desc-mute iconfont icon-mute" _v-c7cde232="" title="false" style="display: none;"></div> <span _v-c7cde232="" title="service" style="display: none;">:</span> <span _v-c7cde232="">挖掘最优质的互联网技术</span> </div>
								</div>
							</div>
							<div class="handle" _v-c7cde232="">
								<div class="handle-unread" _v-c7cde232="" style="display: none; touch-action: manipulation; user-select: none; -webkit-user-drag: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);">标为未读</div>
								<div class="handle-unread" _v-c7cde232="" style="touch-action: manipulation; user-select: none; -webkit-user-drag: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);">标为已读</div>
								<div class="handle-del" _v-c7cde232="" style="touch-action: manipulation; user-select: none; -webkit-user-drag: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);">删除</div>
							</div>
						</li>
						<li class="item _line-fine chat-item-transition" _v-c7cde232="">
							<div class="info" _v-c7cde232="" style="touch-action: pan-y; user-select: none; -webkit-user-drag: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);">
								<div class="ico-box" _v-c7cde232=""> <i _v-c7cde232="" class="_news-count" style="display: none;">0</i>
									<div class="ico" _v-c7cde232=""> <img alt="pic" _v-c7cde232="" src="//tb1.bdstatic.com/tb/cms/ngmis/file_1453447545023.jpg"> </div>
								</div>
								<div class="desc" _v-c7cde232="">
									<div class="desc-time" _v-c7cde232="">16:26</div>
									<div class="desc-title" _v-c7cde232="">饿了么</div>
									<div class="desc-message" _v-c7cde232="">
										<div class="desc-mute iconfont icon-mute" _v-c7cde232="" title="false" style="display: none;"></div> <span _v-c7cde232="" title="service" style="display: none;">:</span> <span _v-c7cde232="">饿了么网上订餐平台</span> </div>
								</div>
							</div>
							<div class="handle" _v-c7cde232="">
								<div class="handle-unread" _v-c7cde232="" style="touch-action: manipulation; user-select: none; -webkit-user-drag: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);">标为未读</div>
								<div class="handle-unread" _v-c7cde232="" style="display: none; touch-action: manipulation; user-select: none; -webkit-user-drag: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);">标为已读</div>
								<div class="handle-del" _v-c7cde232="" style="touch-action: manipulation; user-select: none; -webkit-user-drag: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);">删除</div>
							</div>
						</li>
					</ul>
				</div>
				<div class="_full_router component-chat-dialogue cover-transition" _v-afd9c2a0="" _v-c7cde232="">
					<div class="_full_inner" _v-afd9c2a0="">
						<div class="_cover-top" _v-f50d4fca="" _v-afd9c2a0="">
							<div class="top-back" _v-f50d4fca="">
								<div class="_ellipsis iconfont icon-return-arrow v-link-active" _v-f50d4fca="">微信</div>
							</div>
							<div class="top-other" _v-f50d4fca="">
								<div class="_align-right" _v-f50d4fca=""> <span class="iconfont icon-chat-friends" _v-f50d4fca=""></span> </div>
							</div>
							<div class="top-title _effect" _v-f50d4fca="">
								<p class="_effect" slot="center" _v-afd9c2a0=""> <span class="top-title__text _ellipsis" _v-afd9c2a0="">杨涛</span> <span class="top-title__num parentheses" _v-afd9c2a0="" style="display: none;">1</span> <span class="iconfont icon-mute" _v-afd9c2a0="" style="display: none;"></span> </p>
							</div>
						</div>
						<div class="_cover-content _effect" _v-afd9c2a0="">
							<section class="dialogue-section" _v-afd9c2a0="">
								<div class="dialogue-section-inner" _v-afd9c2a0="">
									<div class="dialogue-item dialogue-item--others" _v-afd9c2a0=""> </div>
									<div class="dialogue-item dialogue-item--time" _v-afd9c2a0=""> </div>
									<div class="dialogue-item dialogue-item--self" _v-afd9c2a0=""> </div>
								</div>
							</section>
							<footer class="dialogue-footer" _v-afd9c2a0="">
								<div class="component-dialogue-bar-person" _v-afd9c2a0=""> <span class="iconfont icon-dialogue-jianpan" style="display: none; touch-action: manipulation; user-select: none; -webkit-user-drag: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);"></span> <span class="iconfont icon-dialogue-voice" style="touch-action: manipulation; user-select: none; -webkit-user-drag: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);"></span>
									<div class="chat-way" style="display: none;">
										<div class="chat-say" style="touch-action: auto; user-select: none; -webkit-user-drag: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);"> <span class="one">按住 说话</span> <span class="two">松开 结束</span> </div>
									</div>
									<div class="chat-way"> <input class="chat-txt" type="text"> </div> <span class="expression iconfont icon-dialogue-smile"></span> <span class="more iconfont icon-dialogue-jia"></span>
									<div class="recording" style="display: none;">
										<div class="recording-voice" style="display: none;">
											<div class="voice-inner">
												<div class="voice-icon"></div>
												<div class="voice-volume"> <span></span> <span></span> <span></span> <span></span> <span></span> <span></span> <span></span> <span></span> <span></span> </div>
											</div>
											<p>手指上划,取消发送</p>
										</div>
										<div class="recording-cancel" style="display: none;">
											<div class="cancel-inner"></div>
											<p>松开手指,取消发送</p>
										</div>
									</div>
								</div>
							</footer>
						</div>
					</div>
				</div>
			</section>
			<footer class="app-footer _line-fine" _v-c8294afa="">
				<div class="_effect _effect--50" _v-c8294afa="">
					<nav _v-26a78bfd="" _v-c8294afa="">
						<dl _v-26a78bfd="" class="v-link-active"> <dt class="iconfont icon-wechat" _v-26a78bfd=""> <i _v-26a78bfd="" class="_news-count">12</i> </dt>
							<dd _v-26a78bfd="">微信</dd>
						</dl>
						<dl _v-26a78bfd=""> <dt class="iconfont icon-contact" _v-26a78bfd="">  </dt>
							<dd _v-26a78bfd="">通讯录</dd>
						</dl>
						<dl _v-26a78bfd=""> <dt class="iconfont icon-find" _v-26a78bfd=""> <i _v-26a78bfd="" class="_news-dot">99</i> </dt>
							<dd _v-26a78bfd="">发现</dd>
						</dl>
						<dl _v-26a78bfd=""> <dt class="iconfont icon-me" _v-26a78bfd="">  </dt>
							<dd _v-26a78bfd="">我</dd>
						</dl>
					</nav>
				</div>
			</footer>
			<section class="welcome welcome-transition" _v-c8294afa="" style="display: none;"></section>
			<section class="mobile-tips" style="display: none;" _v-c8294afa="">
				<div class="mobile-tips-inner" _v-c8294afa="">
					<div class="mobile-model" _v-c8294afa=""> <img src="/static/img/mobile.3c6662f.png" alt="" _v-c8294afa=""></div>
					<p _v-c8294afa=""><br _v-c8294afa="">为保证最佳用户体验,<br _v-c8294afa=""></p>
					<p class="_align-left" _v-c8294afa="">1.请用chrome移动设备调试模式(F12)下打开</p>
					<p class="_align-left" _v-c8294afa="">2.手机浏览器访问</p> <br _v-c8294afa=""> <button class="weui_btn weui_btn_mini weui_btn_primary" _v-c8294afa="" style="touch-action: manipulation; user-select: none; -webkit-user-drag: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);">关闭</button> </div>
			</section>
		</div>
	`
}
